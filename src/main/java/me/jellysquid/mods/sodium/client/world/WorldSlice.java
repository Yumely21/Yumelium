...
public class WorldSlice implements SodiumBlockAccess {
    ...
    /**
     * Bridges Embeddium's {@link BiomeColorSource}-based biome color lookup to 1.12.2's
     * {@link BiomeColorHelper}. Returns an ARGB color.
     * TODO(yumelium): avoid the per-call BlockPos allocation and route through {@link BiomeColorCache} for speed.
     */
    public int getColor(BiomeColorSource source, int x, int y, int z) {
        BlockPos pos = new BlockPos(x, y, z);
        switch (source) {
            case GRASS:   return BiomeColorHelper.getGrassColorAtPos(this, pos);
            case FOLIAGE: return BiomeColorHelper.getFoliageColorAtPos(this, pos);
            case WATER:   return BiomeColorHelper.getWaterColorAtPos(this, pos);
            default:      return -1;
        }
    }

    /**
     * Releases the cloned chunk sections captured by this slice so the section cache can recycle them.
     * Called by the mesher (BlockRenderCache#cleanup) when a chunk build finishes.
     */
    public void reset() {
        ClonedChunkSection[] sections = this.sections;
        if (sections != null) {
