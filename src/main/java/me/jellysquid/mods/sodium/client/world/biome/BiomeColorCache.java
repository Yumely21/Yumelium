...
public class BiomeColorCache {
    ...
    public BiomeColorCache(BiomeColorHelper.ColorResolver resolver, WorldSlice slice) {
        ...
        this.slice = slice;
        // Biome-blend radius (blocks) from the options, CLAMPED to [0, 14]. WorldSlice captures one neighbouring chunk of
        // biome data (NEIGHBOR_CHUNK_RADIUS = 1 → the 3x3 chunk region), which fully covers a blend radius up to 14 — the
        // slider's max. The clamp guards against a hand-edited config value > 14, which would only sample the PLAINS
        // fallback at the fringe (inaccurate) and grow the `cache` array (dim = 16 + (radius+2)*2) for no benefit.
        this.radius = Math.max(0, Math.min(14, me.jellysquid.mods.sodium.client.SodiumClientMod.options().quality.biomeBlendRadius));
