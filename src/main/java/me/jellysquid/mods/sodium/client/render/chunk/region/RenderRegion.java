...
public class RenderRegion {
    ...
    /**
     * A SECOND render list, for the Iris shadow pass.
     *
     * <p>The shadow pass needs a different section set from the camera's: shadows are cast by whatever sits between a
     * surface and the sun, which is usually NOT in the camera's frustum. It cannot share {@link #renderList} because the
     * camera's list is built before the shadow pass and consumed after it, and a {@code ChunkRenderList} holds exactly
     * one frame's worth of sections — refilling it for the light POV would destroy the camera's set mid-frame.</p>
     */
    private final ChunkRenderList shadowRenderList;
    ...
    public RenderRegion(int x, int y, int z, StagingBuffer stagingBuffer) {
        ...
        this.shadowRenderList = new ChunkRenderList(this);
    }
    ...
    /** @see #shadowRenderList */
    public ChunkRenderList getShadowRenderList() {
    ...
    public static class DeviceResources {
        ...
        public DeviceResources(CommandList commandList, StagingBuffer stagingBuffer) {
            ...
            if(!SodiumClientMod.canUseVanillaVertices()) {
                // Iris: match the active compact stride (24 with a_Normal when shaders are on, else 20). A renderer
                // reload recreates all regions, so every arena is allocated at the stride matching the current mode.
                stride = ChunkMeshFormats.activeCompact().getVertexFormat().getStride();
            } else {
