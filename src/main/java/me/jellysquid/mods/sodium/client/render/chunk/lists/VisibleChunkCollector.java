...
public class VisibleChunkCollector implements OcclusionCuller.Visitor {
    ...
    /** Collect into each region's SHADOW render list instead of its camera one — see {@link RenderRegion#getShadowRenderList()}. */
    private final boolean forShadow;

    public VisibleChunkCollector(int frame) {
        this(frame, false);
    }

    public VisibleChunkCollector(int frame, boolean forShadow) {
        this.frame = frame;
        ...
        this.ignoreQueueSizeLimit = false;
    ...
    public void visit(RenderSection section, boolean visible) {
        ...
        ChunkRenderList renderList = this.forShadow ? region.getShadowRenderList() : region.getRenderList();
