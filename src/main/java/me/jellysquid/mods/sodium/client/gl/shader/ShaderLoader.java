...
public class ShaderLoader {
    ...
    /**
     * Compiles a shader from an in-memory source string (rather than a classpath resource), running it through the same
     * {@code #import}/constant preprocessing as {@link #loadShader}. Used by the Iris/Oculus port to compile a shader
     * pack's terrain program after it has been transformed to Sodium's chunk-shader dialect (M5).
     */
    public static GlShader loadShaderFromSource(ShaderType type, ResourceLocation name, String source, ShaderConstants constants) {
        return new GlShader(type, name, ShaderParser.parseShader(source, constants));
    }
