...
public class FlatLightPipeline implements LightPipeline {
    ...
    public void calculate(ModelQuadView quad, BlockPos pos, QuadLightData out, EnumFacing cullFace, EnumFacing face, boolean shade) {
        ...
        // With shaders + oldLighting=false, br lands in the vertex ALPHA the pack reads as vanillaAO — it must be
        // pure AO. Baking the vanilla diffuse face shade here TOO (the pack applies its own directionShade)
        // double-darkened down/side faces (END island undersides measured 31% darker than real 1.20.1 Iris).
        boolean shadeM = shade
                && !com.yumelium.yumelium.shaders.pipeline.IrisPipeline.instance().stripVanillaFaceShade();
        Arrays.fill(out.br, this.lightCache.getWorld().getBrightness(face, shadeM));
    }
