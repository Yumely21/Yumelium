...
public abstract class BlendedColorProvider<T> implements ColorProvider<T> {
    private static boolean shouldUseVertexBlending = true;

    private final BlockPos.MutableBlockPos cursor = new BlockPos.MutableBlockPos();

    public static void checkBlendingEnabled() {
        // Smooth per-vertex (bilinear) blending is the outer layer of biome blending; the inner layer is the radius-based
        // biome-colour average in BiomeColorCache. A biome-blend radius of 0 means "off", so disable BOTH — otherwise the
        // vertex blending would keep smoothing biome-colour transitions even at radius 0.
        shouldUseVertexBlending = me.jellysquid.mods.sodium.client.SodiumClientMod.options().quality.biomeBlendRadius > 0;
    }
    ...
    private int getVertexColor(WorldSlice world, BlockPos blockPos, ModelQuadView quad, int vertexIndex) {
        ...
        final int c00 = this.getColor(world, neighborPos.setPos(worldIntX + 0, worldIntY, worldIntZ + 0));
        final int c01 = this.getColor(world, neighborPos.setPos(worldIntX + 0, worldIntY, worldIntZ + 1));
        final int c10 = this.getColor(world, neighborPos.setPos(worldIntX + 1, worldIntY, worldIntZ + 0));
        final int c11 = this.getColor(world, neighborPos.setPos(worldIntX + 1, worldIntY, worldIntZ + 1));
