...
public record Dim2i(int x, int y, int width, int height) implements Point2i {
    // NOTE(yumelium): getter aliases so Vintagium's ported GUI (which uses getOriginX/getWidth-style names) compiles
    // against this Embeddium 0.5.x record (whose canonical accessors are x()/y()/width()/height()).
