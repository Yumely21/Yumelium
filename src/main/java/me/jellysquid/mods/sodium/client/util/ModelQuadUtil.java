...
public class ModelQuadUtil {
    ...
    public static ModelQuadFacing findNormalFace(float x, float y, float z) {
        ...
        for (EnumFacing face : DirectionUtil.ALL_DIRECTIONS) {
            float dot = (x * face.getXOffset()) + (y * face.getYOffset()) + (z * face.getZOffset());
        ...
        // Mth.equal(a, b) does not exist in 1.12.2; inline the epsilon comparison.
        if (closestFace != null && Math.abs(1.0f - maxDot) < 1.0E-5F) {
