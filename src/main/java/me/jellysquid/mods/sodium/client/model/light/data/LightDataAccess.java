...
public abstract class LightDataAccess {
    ...
    protected long compute(int x, int y, int z) {
        ...
        // Dynamic lights: raise the block-light nibble to the dynamic luminance at this position, if brighter.
        int dynamic = com.yumelium.yumelium.client.light.DynamicLightManager.INSTANCE.getLuminance(x, y, z);
        if (dynamic > 0) {
            int blockLight = (lm >> 4) & 0xF;
            if (dynamic > blockLight) {
                lm = (lm & ~(0xF << 4)) | (dynamic << 4);
            }
        }
